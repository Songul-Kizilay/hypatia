...
- **Runtime releases** (`v0.3.110` in the current release candidate) are the
...
One separate claim boundary accepts only user-authored text plus exact persisted
evidence IDs. It stores the ordered accepted-source provenance derived from
that evidence, one of seven epistemic states, and categorical authored
confidence. Preview and history are read-only; recording revalidates every
...
The desktop keeps successful candidates only as ephemeral state for the exact
run. A separate user action may copy one pair's IDs into the manual form, but
never copies rationale into the authored note or calls the provider again.
Cross-run state is cleared and all write guards remain downstream.
The Research tab also renders the persisted run catalog as a read-only selector
...
Sources/evidence, authored analysis, and review/export each repeat one
presentation-only current-run banner. Its question, status, and exact run ID
come exclusively from the already selected immutable run object; the three
...
Research Overview also binds one workflow snapshot to that immutable run. It
reports complete source, evidence, assessment, comparison-note, claim, and
contradiction collection lengths plus the existing status. It does not classify
readiness, infer truth, recommend a conclusion, hide controls, or open another
runtime boundary; empty and invalid selections replace stale stage records.
The full loaded run tuple remains immutable behind a separate filtered view.
One 200-character presentation filter matches a case-insensitive question
...
That visible tuple has four presentation-only orders: update time newest or
oldest, creation time newest, and case-insensitive question order. The chosen
...
sort before clearing only the presentation filters and restoring the selected
row. Missing, stale, or invalid local state changes neither filter nor view and
opens no read or mutation boundary.
One All/Collecting/Completed/Failed/Cancelled status facet composes with the
bounded text predicate before the deterministic sort. It reports visible/total
...
One explicit `Reset view` action overwrites only the three local view controls
with their safe defaults, sorts the full immutable tuple by newest update, and
...
The same snapshot supplies current-assessment coverage. Assessment IDs named by
later superseding records are excluded before current source IDs are intersected
with accepted source IDs. Each accepted source counts once; superseded-only and
...
call.
One compact catalog summary reuses both local membership derivations to report
complete All, Without evidence, and Without current assessment counts. It is
bound when the immutable run snapshot is selected and is not rewritten by facet
changes, so visible membership cannot change the totals. Empty/invalid state
clears stale counts, no quality/readiness is inferred, and no runtime boundary
opens.
One `Show active source` recovery action verifies exact current run, loaded run,
source-catalog, and active-source identity before restoring All sources and the
active row. Only source-bound read-only evidence and assessment presentation is
rehydrated. Catalog totals and authored fields remain unchanged; empty, stale,
or unknown state is refused without a new read, controller, provider, storage,
or mutation path.
One adjacent selected-source record summary reports exact evidence,
assessment-history, and current-assessment counts from that immutable run. It
keeps correction history distinct from current assessment state, ignores foreign
records, clears when source presentation is hidden or invalid, infers no quality
or trust, and opens no runtime or mutation boundary.
That summary also binds a normalized bounded source title, exact source document
ID, and exact immutable run ID. It requires equality with one canonical source
record in the run before presenting provenance or counts, so a changed or
foreign source object cannot borrow the selected run's identity.
The same canonical presentation exposes persisted data taint
`external_untrusted_data` and instruction authority `none`. User-authored
information-trust assessments remain a separate evidence judgment and cannot
change or hide source instruction authority; no new read or mutation path opens.
It also derives complete Unassessed/Low/Medium/High counts from only the
non-superseded assessments belonging to that exact source. Corrected history
remains visible in the history total, foreign records cannot contribute, and
the four-line provenance/safety/records/trust presentation makes no verdict.
The Records line additionally derives unique current-assessment evidence IDs
intersected with the exact source's immutable evidence set. Repeats count once,
superseded and foreign assessments are excluded, unknown IDs are ignored
defensively, and the result is inventory rather than a quality claim.
One `Source details` action renders only canonical immutable provenance: bounded
title, exact source/run IDs, normalized content type, and fetched/accepted
timestamps at seconds precision with timezone offsets. URL and content are
excluded; missing or changed source records are refused without a runtime call.
...
Its accepted source records also populate a read-only bounded-title/exact-ID
selector. Choosing an item changes no form field; separate explicit handoffs
copy only the exact persisted document ID into the assessment field or append
it once to the ordered comparison field. Cross-run snapshots are rejected and
no Brain, storage, provider, network, or mutation boundary is opened.
The chosen source filters persisted evidence from that same immutable run
object. Its read-only labels retain the exact evidence ID beside a bounded
single-line excerpt. Explicit assessment, claim, and comparison handoffs append
only that ID after same-source or comparison-membership checks; duplicate,
stale, over-20 claim, and over-100 comparison choices fail locally. No extra
read, provider, LLM, knowledge-index, persistence, or mutation path is opened.
The same selected source filters user-authored assessments from the loaded run
object. Labels expose current/superseded audit state, bounded text, and exact ID;
the first current record is preferred without changing form values. Explicit
handoffs accept only current same-source records for the manual correction target
or comparison-assessment list, with source membership, duplicate, stale-state,
and 50-ID checks before any local field change. No runtime boundary is opened.
The selected run also supplies all user-authored claims directly to a read-only
selector. Labels expose current/superseded audit state, epistemic state,
categorical confidence, bounded text, and exact ID; the first current record is
preferred without changing form values. Separate predecessor and contradiction
handoffs accept only current records and locally reject stale, duplicate, or
over-two choices without opening any runtime or mutation boundary.
Persisted user-reviewed claim contradictions in the same run snapshot populate
a separate read-only selector. Each label retains the exact claim pair, bounded
authored note, timezone-aware recorded time, and exact contradiction ID. An
explicit handoff replaces only the manual pair field while preserving the note;
stale or invalid state fails locally without opening a runtime boundary.
Persisted source-comparison notes in that snapshot populate another read-only
selector. Its bounded label retains authored text, recorded time, and exact note
ID, while an associated summary exposes every exact source, evidence, and
assessment reference. Three isolated explicit handoffs replace only the matching
manual reference field and preserve authored text and all non-target fields.
...
- package-aware `python -m unittest`: 1,510 tests passed. Explicit `tests.*`
