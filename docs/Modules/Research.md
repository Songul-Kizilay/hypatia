...
- Schema v4 reads v1-v3 run snapshots with later collections absent and empty,
  upgrading a legacy snapshot only on a later successful atomic save.
...
Define an append-only correction/supersession link for authored assessments
without deleting the audit trail. Discovery and selection remain separate from
fetching and may not become unattended crawling. Multi-source comparison must
operate only on separately accepted, indexed, and cited sources.
