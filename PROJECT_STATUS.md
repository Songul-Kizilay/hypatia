...
- **Runtime release `v0.3.103`** is the current executable package and GitHub
  release line.
...
- 1,495 automated tests pass through package-aware discovery.
...
Add one explicit local `Show active source` recovery action for a source hidden
by the coverage view. It must restore All sources and the exact active row while
preserving authored fields, refuse empty/stale state, and open no runtime or
mutation boundary.
