...
- **Runtime release `v0.3.102`** is the current executable package and GitHub
  release line.
...
- 1,492 automated tests pass through package-aware discovery.
...
Add one compact read-only accepted-source coverage catalog summary with All,
Without evidence, and Without current assessment counts from the selected
immutable run. Counts must remain stable while the local source view changes,
infer no quality, and open no runtime or mutation boundary.
