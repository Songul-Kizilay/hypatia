...
class JsonFileResearchRunStoreTests(unittest.TestCase):
    ...
    def test_schema_18_refuses_an_invalid_observation_identity(self) -> None:
        run = ResearchRun(
            run_id="run-1",
            question="Question",
            status=ResearchRunStatus.COLLECTING,
        ...
        document["runs"][0]["sources"][0]["observation_id"] = " "
        self.path.write_text(json.dumps(document), encoding="utf-8")
    ...
    def test_loads_v1_without_new_collections_and_rewrites_as_current(self) -> None:
        ...
        self.assertEqual(rewritten["schema_version"], 21)
    ...
    def test_loads_v2_without_discoveries_and_rewrites_as_current(self) -> None:
        legacy_document = {
            "schema_version": 2,
            "runs": [
                {
    ...
    def test_loads_v3_without_assessments_and_rewrites_as_current(self) -> None:
        legacy_document = {
            "schema_version": 3,
            "runs": [
                {
    ...
    def test_loads_v4_assessments_without_supersession_and_rewrites_as_current(
        self,
    ) -> None:
        legacy_document = {
            "schema_version": 4,
    ...
    def test_loads_v5_without_comparison_notes_and_rewrites_as_current(self) -> None:
        legacy_document = {
            "schema_version": 5,
            "runs": [
                {
    ...
    def test_loads_v6_without_trust_metadata_and_rewrites_as_current(self) -> None:
        legacy_document = {
            "schema_version": 6,
            "runs": [
                {
    ...
    def test_loads_v7_without_claims_and_rewrites_as_current(self) -> None:
        ...
        for legacy_discovery in legacy_document["runs"][0]["discoveries"]:
            legacy_discovery.pop("candidate_ids", None)
        for legacy_source in legacy_document["runs"][0]["sources"]:
            ...
            legacy_source.pop("discovery_candidate_id", None)
            legacy_source.pop("observation_id", None)
            legacy_source.pop("revalidation_of_observation_id", None)
            legacy_source.pop("revalidation_execution_id", None)
        self.path.write_text(json.dumps(legacy_document), encoding="utf-8")
    ...
    def test_loads_v8_without_claim_contradictions_and_rewrites_as_current(
        self,
    ) -> None:
        run = self._minimal_run()
        self.store.save([run])
    ...
    def _run_with_assessment(self, **judgement: str) -> ResearchRun:
        ...
        assessment = ResearchSourceAssessmentRecord(
            ...
            evidence_type=ResearchSourceEvidenceType(
                judgement.get("evidence_type", "unknown")
            ),
        )
    ...
    def test_v21_keeps_the_operators_evidence_type_answer(self) -> None:
        stored = self._run_with_assessment(
            usefulness="useful",
        ...
        assessment = loaded.assessments[0]
        self.assertEqual(assessment.evidence_type, ResearchSourceEvidenceType.PRIMARY)
        self.assertEqual(assessment.usefulness, ResearchSourceUsefulness.USEFUL)

    def test_a_v20_assessment_loads_evidence_type_as_unknown_not_invented(
        self,
    ) -> None:
        ...
        legacy["runs"][0]["assessments"][0].pop("evidence_type")
        self.path.write_text(json.dumps(legacy), encoding="utf-8")

        [loaded] = self.store.load()

        assessment = loaded.assessments[0]
        self.assertEqual(assessment.evidence_type, ResearchSourceEvidenceType.UNKNOWN)
        # The dimensions born earlier are unaffected by this later one's absence.
        self.assertEqual(assessment.usefulness, ResearchSourceUsefulness.USEFUL)

    def test_an_evidence_type_this_build_cannot_read_fails_the_load_at_v21(
        self,
    ) -> None:
        """Showing it as never made would be worse than refusing to open it."""
        self.store.save([self._run_with_assessment(usefulness="useful")])
        document = json.loads(self.path.read_text(encoding="utf-8"))
        document["runs"][0]["assessments"][0]["evidence_type"] = "primary-source"
        self.path.write_text(json.dumps(document), encoding="utf-8")

        with self.assertRaises(ResearchError):
            self.store.load()

    def test_an_assessment_missing_evidence_type_at_exactly_v21_fails_closed(
        self,
    ) -> None:
        self.store.save([self._run_with_assessment(usefulness="useful")])
        document = json.loads(self.path.read_text(encoding="utf-8"))
        self.assertEqual(document["schema_version"], 21)
        document["runs"][0]["assessments"][0].pop("evidence_type")
        self.path.write_text(json.dumps(document), encoding="utf-8")
    ...
    def test_a_v10_assessment_loads_without_a_judgement_being_invented(self) -> None:
        ...
        legacy.pop("source_revalidations")
        legacy["runs"][0].pop("comparison_reviews")
        for legacy_discovery in legacy["runs"][0]["discoveries"]:
            legacy_discovery.pop("candidate_ids", None)
