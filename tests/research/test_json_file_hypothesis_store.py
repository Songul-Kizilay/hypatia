...
def hypothesis(hypothesis_id: str = "hypothesis-1") -> ResearchHypothesis:
    return ResearchHypothesis(
        hypothesis_id=hypothesis_id,
        run_id="run-1",
        statement="The rings formed recently.",
        discriminating_test="A younger measured age than expected.",
        created_at=RECORDED_AT,
        updated_at=RECORDED_AT,
    )


class JsonFileHypothesisStoreFailurePathTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_truncated_valid_prefix_on_load_fails_safely(self) -> None:
        self.store.save([hypothesis("hypothesis-1"), hypothesis("hypothesis-2")])
    ...
    def test_partial_write_failure_preserves_previous_snapshot_byte_for_byte(
        ...
        self.store.save([hypothesis("hypothesis-original")])
        ...
        with (
            ...
            self.store.save([hypothesis("hypothesis-replacement")])
    ...
    def test_cleanup_unlink_failure_does_not_mask_the_original_research_error(
        ...
        with (
            patch(
                "research.JsonFileHypothesisStore.os.replace",
