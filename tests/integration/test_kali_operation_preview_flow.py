...
class KaliOperationPreviewFlowTests(unittest.TestCase):
    ...
    def test_operation_run_routes_only_with_injected_runtime_and_adapter(
        ...
        self.assertIsNotNone(response.kali_operation_evidence_candidate)
        candidate = response.kali_operation_evidence_candidate
        assert candidate is not None
        self.assertEqual(candidate.candidate_lines, ("192.0.2.10",))
        ...
        self.assertIn("Evidence candidate:", response.message)
        self.assertIn("candidate[0]: 192.0.2.10", response.message)
